...
import {queryUsercopyList} from "@/pages/oee/usercopy/service";
...
const TableList: React.FC<{}> = () => {
  ...
  const [Usercopylist, setUsercopylist] = useState({ data: [] });
      ...
      renderFormItem:()=>{
        return(
            ...
            >
              <Select
                placeholder="请选择用户..."
                onPopupScroll={handlePopupScrollUsercopy}
                allowClear
                showSearch
                optionFilterProp="children"
              >
                {Usercopylist.data.length &&
                  Usercopylist.data.map((obj) => {
  ...
  const getUsercopylist = async (obj) => {
    const Usercopydata = await queryUsercopyList(obj);
    if (Usercopydata.success) {
      const {data} = Usercopylist
      Usercopydata.data = data.concat(Usercopydata.data)
      setUsercopylist(Usercopydata);
    }
  };
useEffect(()=>{getUsercopylist({pageindex:1})},[])
  const handlePopupScrollUsercopy = async (e) => {
    const { current, pagecount } = Usercopylist;
    e.persist();
    const { target } = e;
    if (
      ...
      getUsercopylist({ current: current + 1 });
    }
  };
  const handleUpdate = ()=>{
