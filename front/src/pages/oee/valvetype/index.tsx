...
import { queryValvetypeList, updateValvetype , addValvetype , removeValvetype  } from './service';
...
const handleRemove = async (selectedRows: TableListItem[]) => {
  ...
  try {
    const res = await removeValvetype({
...
const TableList: React.FC<{}> = () => {
  ...
  const handleUpdate = ()=>{
    ...
    form
      ...
      try{
        values.id = id
        const res=await updateValvetype({...values})
        if(res.success){
          hide()
  ...
  const handleAdd = ()=>{
    form
      ...
      const hide=message.loading('正在提交...')
      try{
        const res=await addValvetype({...values})
        if(res.success){
          hide()
  ...
  return (
    <PageHeaderWrapper>
      <ProTable<TableListItem>
        ...
        request={(params, sorter, filter) => queryValvetypeList({ ...params, sorter, filter })}
        columns={columns}
      />
      <Modal
        title="新建阀类型"
